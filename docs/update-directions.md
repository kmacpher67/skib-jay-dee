...
- **v0.4.21 (real code):** the Deaths pill now opens a modal that shows the latest capture records with timestamps and level names, backed by a new `deathsHistory` array in the cookie profile. `GAME_ITERATION` is now `v0.4.21`, and the current version log/smoke suite were updated to reflect it. The next open backlog item is the game identity / multiple save slots work.
