...
- **v0.4.20-plan / v0.4.21-plan (docs-only, most recent session):** reviewed the already-drafted `roadmap-handoff-v0.4.20-plan.md` (dead `initialSheebs = 200` cleanup + sheebs penalty on capture) and `roadmap-handoff-v0.4.21-plan.md` (deaths history log) for handoff readiness — both matched their `docs/roadmap.md` backlog items exactly and needed no changes; confirmed ready to dispatch as-is. Also captured a new direct pacing note from Ken ("level upgrades are still too fast, maybe more time running by the player plus level can not increase until there are at least two skibs simultaneously chasing") as a third plan, **`docs/handoffs/roadmap-handoff-v0.4.22-plan.md`**: adds a `MIN_LEVEL_SECONDS_BEFORE_ADVANCE` elapsed-time floor and a `this.chasers.length >= 2` floor to the non-Pipeworks level-advance check in `GameEngine.js:834`, AND'd with the existing skreem threshold. Pipeworks's own clear gate (5 simultaneous chasers) already exceeds "two skibs" and is untouched. This resolves and supersedes the previously-open "Tune the level-1 → Pipeworks advance threshold" backlog item, now written up with the full design and a copy-paste Code Monkey block. Recommended order for the next coding session: v0.4.20 → v0.4.21 → v0.4.22 (20 and 22 both touch `GameEngine.js`, so doing 20 first avoids a merge conflict; no other real dependency between the three). No code changed this session, `GAME_ITERATION` stays `v0.4.19`.
