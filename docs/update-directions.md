...
- Or pick up the one remaining unblocked v0.4.1-plan item: wire runner
  poses to game state (`RUNNER_FACE_POOL` / `randomFaces()` in
  `frontend/src/gameContent.js`) — swap to `jayden-getting-captured` on
  the jump-scare beat and `jayden-captured` on the caught/"YOU DIED"
  screen. (Sky-Diver landed this session; the shared-chaser-face
  randomization bug landed in v0.4.2-plan; the second Yoodeling Unc pose
  is still blocked on the user saving the file.) See
  `docs/handoffs/roadmap-handoff-v0.4.4.md` for exact file/line
