...
- **Planning refocus (docs-only, this session):** reviewed `docs/handoffs/roadmap-handoff-v0.4.37-plan.md` against the backlog and shifted the next handoff toward a content-first front-end polish pass: richer dialog, badge flavor, map callouts, and a small menu brag surface. The numbers-only balance work stays on the roadmap as a later follow-up.
...
- Planning-only review: the current maps are mechanically fine but need stronger landmark identity, so `docs/interactive-content-pack.md` now seeds the next funny runner/chaser item pack and secret awards. The refined `v0.4.37-plan` turns that into the next fun-first slice.
...
- Next unclaimed, unblocked items from the backlog: the **content-first fun pass** is now the obvious next slice, and the separate **balance follow-up** stays on deck behind it. The badges/rewards system and the Schleimy Potion/Micro-Skib items are still blocked on product decisions from Ken — see their entries in `docs/roadmap.md`. After the fun-first pass, `docs/interactive-content-pack.md` stays the source of truth for any extra secret-item / award ideas.
...
- The current fun-first planning thread is **v0.4.37-plan**: dialog seasoning, badge flavor, map personality, and a small menu brag surface. The follow-on content catalog lives in `docs/interactive-content-pack.md`, and the balance-number pass is intentionally parked behind this slice.
