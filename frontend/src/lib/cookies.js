...
export function normalizeProfile(profile = {}) {
  ...
  return {
    ...
    difficulty: ['noob', 'casual', '4chan-st'].includes(profile.difficulty) ? profile.difficulty : 'casual',
    ...
    levelClearCounts: typeof profile.levelClearCounts === 'object' && profile.levelClearCounts !== null ? profile.levelClearCounts : {},
    highestLevel: Number.isFinite(profile.highestLevel) ? Math.max(1, Math.floor(profile.highestLevel)) : 1,
    highestUnlockedStartLevel: Number.isFinite(profile.highestUnlockedStartLevel) ? Math.max(1, Math.floor(profile.highestUnlockedStartLevel)) : 1,
